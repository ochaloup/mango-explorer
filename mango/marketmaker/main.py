...
from mango.marketmaking.orderchain.chkpchains import get_simple_orderchain
...
def cleanup(
    ...
    builder: mango.MarketInstructionBuilder = mango.create_market_instruction_builder(
        context, wallet, account, market, dry_run
    )
...
def main():
    ...
    wallet = mango.Wallet.from_command_line_parameters(args) or mango.Wallet.load(
        cfg.account.key_file
    )
    group = mango.Group.load(context, context.group_address)
    try:
        account = mango.Account.load_for_owner_by_address(context, wallet.address, group, None)
    ...
    try:
        ...
        if market.quote != group.shared_quote_token:
            raise Exception(
                ...
                f" {group.shared_quote_token.symbol}/{group.shared_quote_token.mint},"
        ...
        if account is not None:
            ...
            mango.InstrumentValue.report(net_values, logging.info)

        manager.open()

        def on_next(_):
            return market_maker.pulse(context, model_state_builder.build(context))

        pulse_disposable = (
            rx.interval(cfg.marketmaker.poll_interval_seconds)
            .pipe(
